class LevelManager {
    ...
    saveLevel(name, sceneData) {
        this.levels[name] = sceneData;
        console.log(`Level '${name}' saved.`, sceneData);
    }
...
class BabylonSceneManager {
    ...
    createBox(name, x, y, z) {
        ...
        boxMesh.metadata = { creationType: 'box', creationParams: { name, x, y, z }, isLevelObject: true };
        return boxMesh;
    }
    ...
    createSphere(name, x, y, z) {
        ...
        sphereMesh.metadata = { creationType: 'sphere', creationParams: { name, x, y, z }, isLevelObject: true };
        return sphereMesh;
    }
    ...
    async importModel(name, url, x, y, z) {
        ...
        if (result.meshes.length > 0) {
            ...
            rootMesh.metadata = { creationType: 'model', creationParams: { name, url, x, y, z }, isLevelObject: true };
            return rootMesh;
        }
        return null;
    }
    ...
    animateProperty(target, propertyPath, endValue, duration, loopMode = 'CONSTANT') {
        let mesh;
        if (typeof target === 'string') {
        ...
        if (!mesh) {
            console.error("Animation target not found:", target);
            return;
        }
        ...
        let animationType = BABYLON.Animation.ANIMATIONTYPE_FLOAT;
        if (propertyPath.toLowerCase().includes('color')) {
            animationType = BABYLON.Animation.ANIMATIONTYPE_COLOR3;
            endValue = BABYLON.Color3.FromHexString(endValue);
        }

        const babylonLoopMode = BABYLON.Animation['ANIMATIONLOOPMODE_' + loopMode.toUpperCase()];

        const animation = new BABYLON.Animation(
            "propertyAnimation_" + propertyPath,
            propertyPath,
            frameRate,
            animationType,
            babylonLoopMode
        ...
        const getProperty = (obj, path) => path.split('.').reduce((o, p) => (o ? o[p] : undefined), obj);
        const startValue = getProperty(mesh, propertyPath);

        if (startValue === undefined) {
            console.error(`Property "${propertyPath}" not found on target object.`, mesh);
            return;
        }

        const keyFrames = [];
        keyFrames.push({ frame: 0, value: startValue });
        keyFrames.push({ frame: totalFrames, value: endValue });

        animation.setKeys(keyFrames);

        this.scene.stopAnimation(mesh, animation.name);
        mesh.animations = mesh.animations ? mesh.animations.filter(a => a.name !== animation.name) : [];
        mesh.animations.push(animation);
        this.scene.beginAnimation(mesh, 0, totalFrames, loopMode !== 'CONSTANT');
    }
    ...
    enablePhysics(target, mass, impostorType = 'BoxImpostor') {
        ...
        if (mesh) {
            ...
            mesh.physicsImpostor = new BABYLON.PhysicsImpostor(mesh, impostor, { mass: mass, restitution: 0.9 }, this.scene);
            // Store physics info in metadata
            if (mesh.metadata) {
                mesh.metadata.physics = { mass: mass, impostorType: impostorType };
            }
        }
    }
    ...
    setAsPlayer(target) {
        ...
        if (mesh) {
            this.player = mesh;
            // Mark the player object so it's not treated as a regular level object
            if (mesh.metadata) {
                mesh.metadata.isLevelObject = false;
            }
        }
    }
    ...
    saveLevel(name) {
        ...
        this.scene.meshes.forEach(mesh => {
            if (mesh.metadata && mesh.metadata.isLevelObject) {
                const data = {
                    name: mesh.name,
                    position: mesh.position.asArray(),
                    rotation: mesh.rotation.asArray(),
                    scaling: mesh.scaling.asArray(),
                    metadata: mesh.metadata // Save original creation info
                };
                sceneData.push(data);
            }
        });
        this.levelManager.saveLevel(name, sceneData);
    }
    ...
    async loadLevel(name) {
        ...
        // Collect meshes to be disposed
        const meshesToDispose = this.scene.meshes.filter(mesh => mesh.metadata && mesh.metadata.isLevelObject);
        meshesToDispose.forEach(mesh => {
            // Also remove from the objects map
            if (this.objects[mesh.name]) {
                delete this.objects[mesh.name];
            }
            mesh.dispose();
        });

        const creationPromises = levelData.map(data => {
            const params = data.metadata.creationParams;
            switch (data.metadata.creationType) {
                case 'box':
                    // Use initial creation params for basic setup, but saved data for final state
                    return Promise.resolve(this.createBox(params.name, 0, 0, 0));
                case 'sphere':
                    return Promise.resolve(this.createSphere(params.name, 0, 0, 0));
                case 'model':
                    // Await the async import
                    return this.importModel(params.name, params.url, 0, 0, 0);
                default:
                    return Promise.resolve(null);
            }
        });

        const newMeshes = await Promise.all(creationPromises);

        newMeshes.forEach((newMesh, index) => {
            if (newMesh) {
                const data = levelData[index];
                newMesh.position = BABYLON.Vector3.FromArray(data.position);
                newMesh.rotation = BABYLON.Vector3.FromArray(data.rotation);
                newMesh.scaling = BABYLON.Vector3.FromArray(data.scaling);

                // Re-apply physics if they were saved
                if (data.metadata.physics) {
                    this.enablePhysics(newMesh, data.metadata.physics.mass, data.metadata.physics.impostorType);
                }
            }
        });
    }
...
Blockly.Themes.DigitalEducationSafety = Blockly.Theme.defineTheme('digital-education-safety', {
    ...
    'fontStyle': { 'family': 'Arial, sans-serif', 'weight': 'normal', 'size': 12 }
        ...
        Blockly.defineBlocksWithJsonArray([
            ...
            // Level Management Blocks
            {
                "type": "save_level",
                "message0": "save current scene as level %1",
                "args0": [
                    { "type": "field_input", "name": "LEVEL_NAME", "text": "level1" }
                ],
                "previousStatement": null,
                "nextStatement": null,
                "colour": "#A55B80",
                "tooltip": "Saves the current state of all dynamic objects as a level.",
                "helpUrl": ""
            },
            {
                "type": "load_level",
                ...
                "nextStatement": null,
                "colour": "#A55B80",
                "tooltip": "Loads a previously saved level.",
                "helpUrl": ""
            },
            {
                "type": "animate_property",
                "message0": "animate property %1 of %2 to value %3 over %4 seconds with loop %5",
                "args0": [
                    { "type": "field_input", "name": "PROPERTY", "text": "position.x" },
                    { "type": "input_value", "name": "OBJECT" },
                    { "type": "input_value", "name": "VALUE" },
                    { "type": "input_value", "name": "DURATION", "check": "Number" },
                    {
                        "type": "field_dropdown", "name": "LOOPMODE",
                        "options": [
                            ["once", "CONSTANT"],
                            ["loop", "CYCLE"],
                            ["ping-pong", "YOYO"]
                ...
                "nextStatement": null,
                "colour": 300,
                "tooltip": "Animates a specific property of an object over a duration.",
                "helpUrl": ""
            }
        ]);
...
`;
            ...
            javascript.javascriptGenerator.forBlock['animate_property'] = function(block, generator) {
                const property = block.getFieldValue('PROPERTY');
                const loopMode = block.getFieldValue('LOOPMODE');
                ...
                return `sceneManager.animateProperty(${object}, '${property}', ${value}, ${duration}, '${loopMode}');\n`;
            };
            ...
            // --- Level Management Generators ---
            javascript.javascriptGenerator.forBlock['save_level'] = function(block, generator) {
                const levelName = block.getFieldValue('LEVEL_NAME');
                return `sceneManager.saveLevel('${levelName}');\n`;
            };

            javascript.javascriptGenerator.forBlock['load_level'] = function(block, generator) {
                const levelName = block.getFieldValue('LEVEL_NAME');
                return `await sceneManager.loadLevel('${levelName}');\n`;
            };
        }
                    ...
                    "blocks": [
                        {
                            ...
                            "next": {
                                "block": {
                                    "type": "create_ground", "fields": { "NAME": "ground" },
                                    "inputs": {
                                    ...
                                    "next": {
                                        "block": {
                                            "type": "variables_set", "fields": { "VAR": { "name": "player", "id": "player_var" } },
                                            "inputs": { "VALUE": { "block": { "type": "create_box", "fields": { "NAME": "player" }, "inputs": { "X": { "block": { "type": "math_number", "fields": { "NUM": 0 } } }, "Y": { "block": { "type": "math_number", "fields": { "NUM": 5 } } }, "Z": { "block": { "type": "math_number", "fields": { "NUM": 0 } } } } } } },
                                            "next": {
                                                "block": {
                                                    "type": "enable_physics", "inputs": { "NAME": { "block": { "type": "variables_get", "fields": { "VAR": { "name": "player", "id": "player_var" } } } }, "MASS": { "block": { "type": "math_number", "fields": { "NUM": 1 } } } },
                                                    "next": {
                                                        "block": {
                                                            "type": "set_as_player", "inputs": { "OBJECT": { "block": { "type": "variables_get", "fields": { "VAR": { "name": "player", "id": "player_var" } } } } },
                                                            "next": {
                                                                "block": {
                                                                    "type": "camera_follow", "inputs": { "OBJECT": { "block": { "type": "variables_get", "fields": { "VAR": { "name": "player", "id": "player_var" } } } } }
                                                                }
                                                            }
                                                        }
                                                    }
                        ...
                        {
                            "type": "variables_set",
                            "fields": { "VAR": { "name": "goal", "id": "goal_var" } },
                            "inputs": { "VALUE": { "block": { "type": "create_box", "fields": { "NAME": "goal" }, "inputs": { "X": { "block": { "type": "math_number", "fields": { "NUM": 8 } } }, "Y": { "block": { "type": "math_number", "fields": { "NUM": 1 } } }, "Z": { "block": { "type": "math_number", "fields": { "NUM": 8 } } } } } } },
                            "next": {
                                "block": {
                                    "type": "change_object_color", "fields": { "COLOR": "#00FF00" }, "inputs": { "NAME": { "block": { "type": "variables_get", "fields": { "VAR": { "name": "goal", "id": "goal_var" } } } } },
                                    "next": {
                                        "block": {
                                            "type": "enable_physics", "inputs": { "NAME": { "block": { "type": "variables_get", "fields": { "VAR": { "name": "goal", "id": "goal_var" } } } }, "MASS": { "block": { "type": "math_number", "fields": { "NUM": 0 } } } }
                                        }
                                    }
                                }
                            }
                        ...
                        {
                            "type": "on_button_press", "fields": { "BUTTON": "A" }, "inputs": { "DO": { "block": { "type": "player_jump", "inputs": { "FORCE": { "block": { "type": "math_number", "fields": { "NUM": 8 } } } } } } }
                        },
                        {
                            "type": "on_button_press", "fields": { "BUTTON": "Left" }, "inputs": { "DO": { "block": { "type": "player_move", "fields": { "DIRECTION": "LEFT" }, "inputs": { "SPEED": { "block": { "type": "math_number", "fields": { "NUM": 5 } } } } } } }
                        },
                        {
                            "type": "on_button_press", "fields": { "BUTTON": "Right" }, "inputs": { "DO": { "block": { "type": "player_move", "fields": { "DIRECTION": "RIGHT" }, "inputs": { "SPEED": { "block": { "type": "math_number", "fields": { "NUM": 5 } } } } } } }
                        },
                        {
                            "type": "on_button_press", "fields": { "BUTTON": "Up" }, "inputs": { "DO": { "block": { "type": "player_move", "fields": { "DIRECTION": "FORWARD" }, "inputs": { "SPEED": { "block": { "type": "math_number", "fields": { "NUM": 5 } } } } } } }
                        },
                        {
                            "type": "on_button_press", "fields": { "BUTTON": "Down" }, "inputs": { "DO": { "block": { "type": "player_move", "fields": { "DIRECTION": "BACKWARD" }, "inputs": { "SPEED": { "block": { "type": "math_number", "fields": { "NUM": 5 } } } } } } }
                        },
                        {
                            "type": "on_collision", "inputs": { "OBJECT1": { "block": { "type": "variables_get", "fields": { "VAR": { "name": "player", "id": "player_var" } } } }, "OBJECT2": { "block": { "type": "variables_get", "fields": { "VAR": { "name": "goal", "id": "goal_var" } } } }, "DO": { "block": { "type": "text_print", "inputs": { "TEXT": { "shadow": { "type": "text", "fields": { "TEXT": "You win!" } } } } } } }
                        }
                    ]
                }
            };
